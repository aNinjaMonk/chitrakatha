...
public class SubTerrain: MonoBehaviour {
	...
	public void RenderDiffuse (ProceduralMaterial Diffuse, int slot) {
		...
		Camera cam = rtObject.transform.Find("RTTSubCamera").camera;
		...
		rtObject.transform.Find("RTTSubPlane").renderer.sharedMaterial.mainTexture = Diffuse.GetTexture ("_MainTex");
		cam.Render(); 
